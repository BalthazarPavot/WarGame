...
public abstract class Unit implements IUnit, Serializable {
	...
	public Unit(int x, int y, GameContext gameContext) {
	...
	public ArrayList<Position> canMove(Map map, Position destination, ArrayList<Unit> units) {
		if (canFly())
			return map.pathByFlying(getPosition(), destination, units);
		return map.pathByWalking(getPosition(), destination, units);
	}
	...
	public void moveTo(Position destination, Map map, ArrayList<Unit> units) {
		ArrayList<Position> path;

		path = canMove(map, destination, units);
		if (path != null)
			setMove(path);
	}

	public void moveTo(int x, int y, Map map, ArrayList<Unit> units) {
		moveTo(new Position(x, y), map, units);
	}
	...
	public ArrayList<Position> movePossibilities(Map map, ArrayList<Unit> units) {
	 ...
	 * @return
	 */
	public boolean canReach(Map map, Position endPos, ArrayList<Unit> units) {
		int distance;
		ArrayList<Position> path = new ArrayList<Position>();

		path = this.canMove(map, endPos, units);
